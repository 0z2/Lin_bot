...
async def send_welcome(message: types.Message, regexp_command):
    ...
    await message.answer(f'Ачивка Voice за <code>{date_of_achive}</code> добавлена 🗣️')
    await dp.bot.send_message(985485455,
                              f"Пользователь {message.from_user.full_name} добавил ачивку Voice за {date_of_achive}")
